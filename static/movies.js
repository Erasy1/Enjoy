(function () {
  ...
  document.querySelectorAll(".arrow").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      const localStep = targetId === "mvRecoTrack" ? stepPoster : stepSmall;
  ...
  async function openMediaDetails(tmdbId, mediaType, titleFallback) {
    ...
    currentMediaType = mediaType === "tv" ? "tv" : "movie";
  ...
  document.addEventListener("click", (e) => {
    ...
    const mediaType = (a.getAttribute("data-media-type") || "movie").trim();
    e.preventDefault();
  ...
  const mvRecoTrack = document.getElementById("mvRecoTrack");
  const mvResTrack = document.getElementById("mvResTrack");
  ...
  async function loadRecommendationRow() {
    ...
    mvRecoTrack.innerHTML = `<div class="empty">Loading...</div>`;
    ...
    try {
      ...
      if (onlyMovies.length) {
        mvRecoTrack.innerHTML = onlyMovies.slice(0, 12).map(posterMiniHTML).join("");
        return;
      }
    } catch (e) {
      console.warn("reco api failed, fallback to top", e);
    }

    try {
      const top = await fetchJSON(`/api/movies/top?kind=top30&lang=ru-RU`);
      const items = Array.isArray(top.items) ? top.items : [];
      mvRecoTrack.innerHTML =
        items.slice(0, 12).map(posterMiniHTML).join("") || `<div class="empty">No recommendations</div>`;
    } catch (e) {
      console.error("fallback top failed", e);
      mvRecoTrack.innerHTML = `<div class="empty">No recommendations</div>`;
    }
  }

  async function loadResults() {
    if (!mvResTrack) return;
    ...
    const url =
      `/api/movies/discover?lang=ru-RU&genres=${encodeURIComponent(genres)}` +
    ...
    try {
      ...
      mvResTrack.innerHTML = items.length ? items.map(cardHTML).join("") : `<div class="empty">No results</div>`;
      scrollToResults();
    } catch (e) {
      console.error("DISCOVER ERROR:", e);
      mvResTrack.innerHTML = `<div class="empty">Discover failed</div>`;
      scrollToResults();
    }
  }
  ...
  async function loadTop(limit) {
    ...
    try {
      ...
      const items = Array.isArray(data.items) ? data.items : [];
      if (items.length) {
        mvResTrack.innerHTML = items.map(cardHTML).join("");
        scrollToResults();
        return;
      }
    } catch (e) {
    ...
    try {
      ...
      const items = Array.isArray(top.items) ? top.items : [];
      mvResTrack.innerHTML = items.length
        ? items.map(cardHTML).join("")
        : `<div class="empty">No results</div>`;
    } catch (e) {
      console.error("fallback top failed", e);
      mvResTrack.innerHTML = `<div class="empty">No results</div>`;
    }
  ...
  (async function init() {
    ...
    try {
      const g = await fetchJSON("/api/genres/movie?lang=ru-RU");
